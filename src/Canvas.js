...
const Canvas = ({ settings, textToAdd, ...rest }) => {
  console.log(textToAdd, "from Canvas")
  ...
  useEffect(() => {
    ...
    drawCanvas(getContext()); // ← Call with no second arg
  }, [textToAdd]);
    ...
    <>
      ...
      >
        <button className="button" type="button" onClick={onImportClick}>
          <img src="assets/export.svg" alt="import" title="import" />
        </button>
        <button
          className="button"
          type="button"
          ...
          disabled={history.current.length === 0}
        >
          <img src="assets/import.svg" alt="export" title="export" />
        </button>
        <input
          ref={importInput}
          className="hidden"
